...
TRADING_COSTS = {
    "XBTUSDT": {"slippage": 0.0002, "fee": 0.0040},  # Updated to 0.40% taker
    "ETHUSDT": {"slippage": 0.0003, "fee": 0.0040},  # Updated to 0.40% taker
    "SOLUSDT": {"slippage": 0.0005, "fee": 0.0040},  # Updated to 0.40% taker
    "ADAUSDT": {"slippage": 0.0012, "fee": 0.0040},  # Updated to 0.40% taker
    "DOTUSDT": {"slippage": 0.0010, "fee": 0.0040},  # Updated to 0.40% taker
    "LTCUSDT": {"slippage": 0.0004, "fee": 0.0040},  # Updated to 0.40% taker
    "XRPUSDT": {"slippage": 0.0010, "fee": 0.0040},  # Updated to 0.40% taker
    "LINKUSDT": {"slippage": 0.0008, "fee": 0.0040}, # Updated to 0.40% taker
    "AVAXUSDT": {"slippage": 0.0015, "fee": 0.0040}, # Updated to 0.40% taker
}

# === AGGRESSIVE TRADING RULES ===
STOP_LOSS_PCT = 0.06  # 6% stop loss (aggressive but safe)
TAKE_PROFIT_LEVELS = [0.04, 0.08, 0.15]  # 4%, 8%, 15%
TAKE_PROFIT_ALLOCATIONS = [0.30, 0.40, 0.30]  # 30%, 40%, 30% exits
MARGIN_OPEN_FEE = 0.0002
MAX_HOLD_HOURS = 18  # Maximum 18 hours per trade

# Minimum profit threshold after ALL costs
MIN_PROFIT_THRESHOLD = 0.008  # Must make at least 0.8% after all costs
...
def create_institutional_labels(df: pd.DataFrame, pair: str, 
    ...
    for i in range(total_rows):
        ...
        for horizon_hours, label_col in [(1, 'institutional_label_1h'), 
                                       (4, 'institutional_label_4h'),
                                       (12, 'institutional_label_12h')]:
...
def main():
    ...
    for pair in PAIRS:
        try:
            # Load feature data
            table_name = f"features_{pair}_1m"
            print(f"\n🔄 Processing {pair}...")
            ...
            df_labeled = create_institutional_labels(df, pair, lookforward_hours=72)
            
            # Analyze label quality
            analyze_institutional_labels(df_labeled, pair)
            
            # Save labeled data
            output_table = f"features_{pair}_1m_institutional"
